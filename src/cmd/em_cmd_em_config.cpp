...
em_cmd_em_config_t::em_cmd_em_config_t(em_cmd_params_t param, dm_easy_mesh_t& dm)
{
    ...
    m_orch_op_idx = 0;
    m_num_orch_desc = 9;
    m_orch_desc[i].op = dm_orch_type_bss_delete;
    m_orch_desc[i].submit = false;
    i++;
    m_orch_desc[i].op = dm_orch_type_topo_sync;
    m_orch_desc[i].submit = true;
    i++;
    // m_orch_desc[i].op = dm_orch_type_ap_cap_query;
    // m_orch_desc[i].submit = true;
    // i++;
    m_orch_desc[i].op = dm_orch_type_channel_pref;
    m_orch_desc[i].submit = true;
    i++;
    //sending policy_cfg req before channel selection, since policy_cfg is per device
    m_orch_desc[i].op = dm_orch_type_policy_cfg;
    m_orch_desc[i].submit = true;
    i++;
    m_orch_desc[i].op = dm_orch_type_channel_sel;
    m_orch_desc[i].submit = true;
    i++;
    m_orch_desc[i].op = dm_orch_type_channel_scan_req;
    m_orch_desc[i].submit = true;
    i++;
    m_orch_desc[i].op = dm_orch_type_topo_update;
    m_orch_desc[i].submit = false;
    i++;
    m_orch_desc[i].op = dm_orch_type_topo_publish;
    m_orch_desc[i].submit = true;
