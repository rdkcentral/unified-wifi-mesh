	...
	for (i = 0; i < em_dev_test_type_max; i++) {
		...
		if (dev_test_info.test_type[i] == em_dev_test_type_ssid) {
			...
			if(!update) {
				...
				cJSON_AddNumberToObject(dev_test_param, "Haul_type:[Fronthault=0,IOT:2]", dev_test_info.haul_type);
			}
	...
	for (i = 0; i < arr_size; i++) {
		...
		if ((tmp = cJSON_GetObjectItem(obj, "Test_enabled")) != NULL) {
			...
			if (dev_test_info.enabled[index] == 0) {
				dev_test_info.test_status[index] = em_dev_test_status_ideal;
...
/* Need to be implemented
		...
		if ((tmp = cJSON_GetObjectItem(obj, "Haul_type"))  != NULL) {
				dev_test_info.haul_type = (em_haul_type_t) (tmp->valuedouble);
		}
*/
	}
}
