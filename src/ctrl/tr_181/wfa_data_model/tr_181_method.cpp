...
bus_error_t tr_181_t::clientassoccontrol_handler(const char *method_name, bus_data_prop_t *input_data,
    bus_data_prop_t *output_data, void *async_handle)
{
    // Standardize input pointer checks
    if (!input_data || (!input_data->is_data_set && input_data->next_data == NULL)) {
    ...
    bus_error_t rc = ctrl->cmd_clientassoccontrol(method_name, input_props, output_data ? &output_props : NULL, async_handle);
