...
short em_policy_cfg_t::create_vendor_policy_cfg_tlv(unsigned char *buff)
{
    ...
    em_printfout(" Vendor Policy cfg total numbers: %zu ", vendor_tlv->num);
...
short em_policy_cfg_t::create_vendor_policy_cfg_tlv(unsigned char *buff)
{
    ...
    unsigned char *cursor = vendor_data->data;
    unsigned char *payload_start = vendor_data->data;
        ...
        if (tlv->type == em_tlv_type_steering_policy) {
            ...
            for(int i = 0; i < vendor_tlv->num; i++)
            {
                ...
                if (data->attr_id == vendor_ext_attr_id_policy_sta_marker) {
                    strncpy(policy.vendor_policy.managed_client_marker, (const char *)data->vendor_data, strlen(reinterpret_cast<char *> (data->vendor_data)) + 1);
                    ...
                    em_printfout(" Recvd client filters cfg, sta_mac : %02X:%02X:%02X:%02X:%02X:%02X ", 
                        policy.vendor_policy.client_filters_policy_cfg.sta_mac[0],
                        policy.vendor_policy.client_filters_policy_cfg.sta_mac[1],
                        policy.vendor_policy.client_filters_policy_cfg.sta_mac[2],
                        policy.vendor_policy.client_filters_policy_cfg.sta_mac[3],
                        policy.vendor_policy.client_filters_policy_cfg.sta_mac[4],
                        policy.vendor_policy.client_filters_policy_cfg.sta_mac[5]);
