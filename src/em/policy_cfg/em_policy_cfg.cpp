...
short em_policy_cfg_t::create_steering_policy_tlv(unsigned char *buff)
{
	...
	len += static_cast<short> (sizeof(unsigned char) + sta_policy->num_sta*sizeof(mac_address_t));
	...
	if (found_match == false) {
		...
		*tmp = 1;
		...
		len += static_cast<short> (sizeof(em_steering_policy_radio_t));
	}
...
short em_policy_cfg_t::create_vendor_policy_cfg_tlv(unsigned char *buff)
{
    ...
    len += static_cast<short> (strlen(policy->m_policy.managed_sta_marker));
...
int em_policy_cfg_t::handle_policy_cfg_req(unsigned char *buff, unsigned int len)
{
    ...
    em_tlv_t    *tlv;
    unsigned int tlv_len, data_len = 0;
    ...
    while ((tlv->type != em_tlv_type_eom) && (tlv_len > 0)) {
        if (tlv->type == em_tlv_type_steering_policy) {
            ...
            data_len += static_cast<unsigned int> (sizeof(steer_pol_sta->num_sta) + (sizeof(mac_addr_t) * steer_pol_sta->num_sta));
            ...
            data_len += static_cast<unsigned int> (sizeof(btm_steer_pol->num_sta) + (sizeof(mac_addr_t) * btm_steer_pol->num_sta));

            policy.steering_policy.radio_num = *(tlv->value + data_len);
            data_len += static_cast<unsigned int> (sizeof(unsigned char));
            ...
            data_len += static_cast<unsigned int> (policy.steering_policy.radio_num * sizeof(em_steering_policy_radio_t));
            ...
            data_len += static_cast<unsigned int> (metrics->radios_num * sizeof(em_metric_rprt_policy_radio_t));
