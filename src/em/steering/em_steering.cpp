...
bool get_target_bss_channel_info(dm_easy_mesh_t *dm, em_cmd_steer_params_t *params)
{
    ...
    if (target_bss == NULL) {
        em_printfout("Target BSS:%s not part of dm (id:%u, num_bss:%u)", util::mac_to_string(params->target).c_str(), dm->get_id(), dm->get_num_bss());
        return false;
    }
...
int em_steering_t::disassoc_non_11v_client(em_sta_info_t *sta_info, bssid_t bssid)
{
    ...
    disassoc_param->params[0].reason = WLAN_REASON_PREV_AUTH_NOT_VALID;
...
int em_steering_t::handle_client_steering_req(unsigned char *buff, unsigned int len)
{
    ...
    if (is_sta_in_steering_policy_list(dm, sta_mac, em_policy_id_type_steering_btm)) {
        ...
        em_printfout("Enforcing Client Assoc control request for sta=%s, bssid=%s", mac_str, dm_easy_mesh_t::macbytes_to_string(source_bssid, bssid_str));
    ...
    //In case of steering Mandate, disassociate non-11v client. If it is steering opportunity, disassociate only if RCPI is less than 80 or disassoc_imminent is true.
...
int em_steering_t::handle_ack_msg(unsigned char *buff, unsigned int len)
{
    ...
    if ((dm != NULL) && (memcmp(hdr->dst, dm->get_agent_al_interface_mac(), MAC_ADDR_LEN) == 0)) {
        memcpy(al_mac, hdr->dst, sizeof(mac_address_t));
    } else {
        memcpy(al_mac, hdr->src, sizeof(mac_address_t));
    }

    get_mgr()->get_all_em_for_al_mac(al_mac, em_radios);
