...
int em_steering_t::send_client_assoc_ctrl_req_msg()
{
    ...
    for (unsigned int i = 0; i < pcmd->m_param.u.disassoc_params.num; i++) {
        ...
        em_disassoc_params_t *disassoc_param =
            &pcmd->m_param.u.disassoc_params.params[i];

        for (unsigned int j = 0; j < dm->m_num_bss; j++) {

            if ((memcmp(disassoc_param->bssid,
                        dm->m_bss[j].m_bss_info.bssid.mac,
                        sizeof(bssid_t)) == 0) &&
                (memcmp(dm->m_bss[j].m_bss_info.ruid.mac,
                ...
                memcpy(&assoc_ctrl.bssid,
                       &disassoc_param->bssid,
                       sizeof(bssid_t));

                assoc_ctrl.count = 1;
                memcpy(&assoc_ctrl.sta_mac,
                       &disassoc_param->sta_mac,
                       sizeof(mac_address_t));

                /*
                 * Logical Handling:
                 * disassoc_time > 0  -> Timed Block
                 * disassoc_time == 0 -> Unblock
                 */

                if (disassoc_param->disassoc_time > 0) {
                    // BLOCK
                    assoc_ctrl.assoc_control  = 0x00;
                    assoc_ctrl.validity_period =
			    htons(static_cast<uint16_t>(disassoc_param->disassoc_time));
                } else {
                    // UNBLOCK
...
int em_steering_t::send_client_assoc_ctrl_req_msg(em_client_assoc_ctrl_req_t *assoc_ctrl)
{
    ...
    short tlv_value_len = static_cast<short>(sizeof(bssid_t) + sizeof(unsigned char) + sizeof(unsigned short) + sizeof(unsigned char) + assoc_ctrl->count * sizeof(mac_address_t));
