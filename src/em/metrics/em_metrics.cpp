...
short em_metrics_t::send_beacon_metrics_query(mac_address_t sta_mac, bssid_t bssid)
{
    ...
    memcpy(tmp, reinterpret_cast<unsigned char *> (&type), sizeof(unsigned short));
    tmp += sizeof(unsigned short);
    len += static_cast<short> (sizeof(unsigned short));
    ...
    tmp += sizeof(em_cmdu_t);
    len += static_cast<short> (sizeof(em_cmdu_t));
    ...
    tmp += (sizeof (em_tlv_t) + static_cast<size_t> (sz));
    len += static_cast<short> (sizeof (em_tlv_t) + static_cast<size_t> (sz));
    ...
    tmp += (sizeof (em_tlv_t));
    len += static_cast<short> (sizeof (em_tlv_t));
...
short em_metrics_t::create_assoc_sta_link_metrics_tlv(unsigned char *buff, mac_address_t sta_mac, const dm_sta_t *const sta)
{
    ...
    if (sta == NULL) {
        ...
        len += static_cast<short> (sizeof(assoc_sta_metrics->sta_mac));

        assoc_sta_metrics->num_bssids = 0;
        len += static_cast<short> (sizeof(assoc_sta_metrics->num_bssids));
        return len;
    }
    else {
        ...
        if ((memcmp(sta->m_sta_info.id, sta_mac, sizeof(mac_address_t)) == 0)) {
            ...
            len += static_cast<short> (sizeof(metrics->bssid));

            metrics->time_delta_ms = 10;//TODO: Pending proper update
            len += static_cast<short> (sizeof(metrics->time_delta_ms));

            metrics->est_mac_data_rate_dl = sta->m_sta_info.last_dl_rate;
            len += static_cast<short> (sizeof(metrics->est_mac_data_rate_dl));

            metrics->est_mac_data_rate_ul = sta->m_sta_info.last_ul_rate;
            len += static_cast<short> (sizeof(metrics->est_mac_data_rate_ul));

            metrics->rcpi = 1;//TODO: Pending proper update
            len += static_cast<short> (sizeof(metrics->rcpi));
        }
    }
    return len;
}
...
short em_metrics_t::create_assoc_ext_sta_link_metrics_tlv(unsigned char *buff, mac_address_t sta_mac, const dm_sta_t *const sta)
{
    ...
    if (sta == NULL) {
        ...
        if ((memcmp(sta->m_sta_info.id, sta_mac, sizeof(mac_address_t)) == 0)) {
            ...
            len += static_cast<short> (sizeof(metrics->last_data_dl_rate));

            metrics->last_data_ul_rate = sta->m_sta_info.last_ul_rate;
            len += static_cast<short> (sizeof(metrics->last_data_ul_rate));

            metrics->util_receive = sta->m_sta_info.util_rx;
            len += static_cast<short> (sizeof(metrics->util_receive));

            metrics->util_transmit = sta->m_sta_info.util_tx;
            len += static_cast<short> (sizeof(metrics->util_transmit));
        }
    }
    return len;
}
...
short em_metrics_t::create_assoc_vendor_sta_link_metrics_tlv(unsigned char *buff, mac_address_t sta_mac, const dm_sta_t *const sta)
{
    ...
    if (sta == NULL) {
        ...
        if ((memcmp(sta->m_sta_info.id, sta_mac, sizeof(mac_address_t)) == 0)) {
            ...
            len += static_cast<short> (sizeof(metrics->packets_received));

            metrics->packets_sent = sta->m_sta_info.pkts_tx;
            len += static_cast<short> (sizeof(metrics->packets_sent));

            metrics->bytes_received = sta->m_sta_info.bytes_rx;
            len += static_cast<short> (sizeof(metrics->bytes_received));

            metrics->bytes_sent = sta->m_sta_info.bytes_tx;
            len += static_cast<short> (sizeof(metrics->bytes_sent));
        }
    }
    return len;
}
...
short em_metrics_t::create_beacon_metrics_query_tlv(unsigned char *buff, mac_address_t sta_mac, bssid_t bssid)
{
    ...
    if (sta == NULL) {
        ...
        len += static_cast<short> (sizeof(beacon_metrics->sta_mac_addr));

        beacon_metrics->op_class = 10;
        len += static_cast<short> (sizeof(beacon_metrics->op_class));

        beacon_metrics->channel_num = 6;
        len += static_cast<short> (sizeof(beacon_metrics->channel_num));

        memcpy(beacon_metrics->bssid, bssid, sizeof(bssid_t));
        len += static_cast<short> (sizeof(beacon_metrics->bssid));

        beacon_metrics-> rprt_detail = 1;
        len += static_cast<short> (sizeof(beacon_metrics-> rprt_detail));

        beacon_metrics->ssid_len = sizeof(ssid);
        len += static_cast<short> (sizeof(beacon_metrics->ssid_len));

        memcpy(beacon_metrics->ssid, ssid, sizeof(ssid));
        len += static_cast<short> (sizeof(ssid));

        beacon_metrics->num_ap_channel_rprt = 2;
        len += static_cast<short> (sizeof(beacon_metrics->num_ap_channel_rprt));
        ...
        for (int i = 0; i < beacon_metrics->num_ap_channel_rprt; i++) {
            ...
            len += static_cast<short> (sizeof(ap_chann_rprt->ap_channel_rprt_len));

            ap_chann_rprt->ap_channel_op_class = 10;
            len += static_cast<short> (sizeof(ap_chann_rprt->ap_channel_op_class));
...
short em_metrics_t::create_beacon_metrics_response_tlv(unsigned char *buff)
{
    ...
    if (sta != NULL) {
        ...
        len += static_cast<short> (sizeof(response->sta_mac_addr));

        response->reserved = 0;
        len += static_cast<short> (sizeof(response->reserved));

        response->meas_rprt_count = static_cast<unsigned char> (sta->m_sta_info.num_beacon_meas_report);
        len += static_cast<short> (sizeof(response->meas_rprt_count));

        memcpy(response->meas_reports, sta->m_sta_info.beacon_report_elem, sta->m_sta_info.beacon_report_len);
        len += static_cast<short> (sta->m_sta_info.beacon_report_len);
    }
