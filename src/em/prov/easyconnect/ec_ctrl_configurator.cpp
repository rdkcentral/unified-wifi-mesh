...
std::pair<uint8_t *, uint16_t> ec_ctrl_configurator_t::create_auth_confirm()
{
    ...
    //     } else {
    //         wrap_attribs = ec_util::add_attrib(wrap_attribs, &wrapped_len, ec_attrib_id_resp_nonce, m_params.nonce_len, m_params.responder_nonce);
    //     }
    //     return std::make_pair(wrap_attribs, wrapped_len);
    // });
...
std::pair<uint8_t *, uint16_t> ec_ctrl_configurator_t::create_recfg_auth_confirm(std::string enrollee_mac)
{
