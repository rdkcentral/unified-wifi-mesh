...
bool ec_ctrl_configurator_t::process_proxy_encap_dpp_msg(em_encap_dpp_t *encap_tlv, uint16_t encap_tlv_len, em_dpp_chirp_value_t *chirp_tlv, uint16_t chirp_tlv_len)
{
    ...
    switch (ec_frame_type) {
        case ec_frame_type_recfg_announcement: {
            did_finish = handle_recfg_announcement(encap_frame, encap_frame_len, dest_mac);
            break;
        }
...
bool ec_ctrl_configurator_t::handle_recfg_announcement(uint8_t *encap_frame, size_t len, uint8_t sa[ETH_ALEN])
{
...
std::pair<uint8_t *, size_t> ec_ctrl_configurator_t::create_recfg_auth_request(const std::string& enrollee_mac)
{
    ...
    m_c_connector = ec_crypto::generate_connector(jwsHeaderObj, jwsPayloadObj, conn_ctx->C_signing_key);

    attribs = ec_util::add_attrib(attribs, &attribs_len, ec_attrib_id_dpp_connector, m_c_connector);
