...
bool ec_ctrl_configurator_t::handle_recfg_auth_response(ec_frame_t *frame, size_t len, uint8_t sa[ETH_ALEN])
{
    ...
    BN_free(e_ctx->m);
    e_ctx->m = BN_new();
    ASSERT_NOT_NULL(e_ctx->m, false, "%s:%d: Could not allocate for shared secret M\n", __func__, __LINE__);
    ...
    // so just don't use it. Additionally, 6.5.4 which describes the creation of the Auth Resp frame says
...
std::pair<uint8_t *, size_t> ec_ctrl_configurator_t::create_recfg_auth_confirm(std::string enrollee_mac, ec_status_code_t dpp_status, uint8_t trans_id)
{
