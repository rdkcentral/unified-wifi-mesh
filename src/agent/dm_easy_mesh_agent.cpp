...
int dm_easy_mesh_agent_t::analyze_link_stats_report(em_bus_event_t *evt, em_cmd_t *pcmd[])
{
    dm_sta_t *sta = NULL;
    em_cmd_t *tmp = NULL;
    em_cmd_params_t *evt_param = NULL;
    ...
    // dm.translate_and_decode_onewifi_subdoc((char *)evt->u.raw_buff, type, "Link Stats Report");
    ...
    //     // dm_easy_mesh_t::macbytes_to_string(sta->m_sta_info.bssid, bss_mac_str);
    //     // dm_easy_mesh_t::macbytes_to_string(sta->m_sta_info.radiomac, radio_mac_str);
    //     em_printfout("Before trasnlate sta %s", util::mac_to_string(sta->m_sta_info.id).c_str());
