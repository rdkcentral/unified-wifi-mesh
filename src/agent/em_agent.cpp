...
void em_agent_t::handle_btm_request_action_frame(em_bus_event_t *evt)
{
    ...
    int num;
    unsigned int state = em_state_agent_unconfigured;
    wifi_bus_desc_t *desc;
...
{
    if (m_data_model.analyze_btm_query_action_frame(evt, NULL, &m_bus_hdl) != 1) {
        em_printfout("analyze_btm_query_action_frame failed");
    }
}
...
void em_agent_t::handle_btm_response_action_frame(em_bus_event_t *evt)
{
    ...
    if (m_orch->is_cmd_type_in_progress(evt) == true) {
        em_printfout("analyze_btm_response_action_frame in progress\n");
    } else if ((num = static_cast<unsigned int>(m_data_model.analyze_btm_response_action_frame(evt, pcmd))) == 0) {
        em_printfout("analyze_btm_response_action_frame failed\n");
    } else if (m_orch->submit_commands(pcmd, num) > 0) {
        em_printfout("submitted handle_btm_response_action_frame command for orchestration\n");
    }
...
void em_agent_t::add_sta_timer(em_sta_timer_t *sta_timer_params)
{
    ...
    long time_sec  = sta_timer_params->duration_ms / EM_SEC_TO_MSEC;
    long time_nsec = (long)(sta_timer_params->duration_ms % EM_SEC_TO_MSEC) * EM_NSEC_TO_SEC/EM_SEC_TO_MSEC;
    sta_timer_params->expire_time.tv_sec  = now.tv_sec  + time_sec;
    sta_timer_params->expire_time.tv_nsec = now.tv_nsec + time_nsec;
    if (sta_timer_params->expire_time.tv_nsec >= EM_NSEC_TO_SEC) {
        sta_timer_params->expire_time.tv_sec++;
        sta_timer_params->expire_time.tv_nsec -= EM_NSEC_TO_SEC;
    }
...
void em_agent_t::handle_disassoc_timer_expired(em_bus_event_t *evt)
{
    ...
    mac_addr_str_t sta_mac_str, bssid_str;
    unsigned char cac_buff[MAX_EM_BUFF_SZ];
    unsigned char disassoc_buff[MAX_EM_BUFF_SZ];
    ...
    sta = m_data_model.find_sta(sta_timer_params->sta_mac, sta_timer_params->source_bssid);
    if (sta != NULL) {
        em_client_assoc_ctrl_req_t *assoc_ctrl_req;

        assoc_ctrl_req = reinterpret_cast<em_client_assoc_ctrl_req_t *>(cac_buff);
        memset(assoc_ctrl_req, 0, sizeof(em_client_assoc_ctrl_req_t));

        memcpy(&assoc_ctrl_req->bssid, sta_timer_params->source_bssid, sizeof(bssid_t));
        assoc_ctrl_req->assoc_control = ASSOC_CONTROL_BLOCK;
        ...
        g_agent.io_process(em_bus_event_type_client_assoc_ctrl_req, reinterpret_cast<unsigned char *>(assoc_ctrl_req),
            sizeof(em_client_assoc_ctrl_req_t));
    } else {
        em_printfout("STA not found for CAC on disassoc timer expiry: sta=%s bssid=%s", sta_mac_str, bssid_str);
    }

    disassoc_param = reinterpret_cast<em_cmd_disassoc_params_t *>(disassoc_buff);
...
void em_agent_t::handle_steering_window_expired(em_bus_event_t *evt)
{
    ...
    if (m_orch->is_cmd_type_in_progress(evt) == true) {
        em_printfout("analyze_steering_window_expired in progress\n");
    } else if ((num = m_data_model.analyze_steering_window_expired(evt, pcmd)) == 0) {
        em_printfout("analyze_steering_window_expired failed\n");
    } else if (m_orch->submit_commands(pcmd, num) > 0) {
        em_printfout("submitted handle_steering_window_expired command for orchestration\n");
    }
}
...
void em_agent_t::input_listener()
{
    ...
    while ((bus_error_val = desc->bus_data_get_fn(&m_bus_hdl, WIFI_WEBCONFIG_INIT_DML_DATA, &data)) != bus_error_success) {
		...
		usleep(RETRY_SLEEP_INTERVAL_IN_MS * EM_SEC_TO_MSEC);
		num_retry++;
