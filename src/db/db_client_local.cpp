...
int db_client_local_t::recreate_db()
{
    if (!m_con) {
...
void *db_client_local_t::execute(const char *query)
{
    if (!m_con) {
...
bool db_client_local_t::next_result(void *ctx)
{
    if (ctx == NULL) {
        return false;
    }
...
void db_client_local_t::free_result(void *ctx)
{
    if (ctx == NULL) {
        return;
    }
...
char *db_client_local_t::get_string(void *ctx, char *str, unsigned int col)
{
    if (ctx == NULL) {
        return NULL;
    }
...
int db_client_local_t::get_number(void *ctx, unsigned int col)
{
    assert(ctx != NULL);
...
int db_client_local_t::connect(const char *path)
{
...
int db_client_local_t::init(const char *path)
{
    if (connect(path) != 0) {
...
db_client_local_t::db_client_local_t()
{
    m_con = NULL;
}
