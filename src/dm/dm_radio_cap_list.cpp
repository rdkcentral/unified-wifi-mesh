...
int dm_radio_cap_list_t::get_config(cJSON *obj_parent, void *parent)
{
...
int dm_radio_cap_list_t::set_config(db_client_t& db_client, const cJSON *obj_arr, void *parent_id)
{
    ...
    for (i = 0; i < size; i++) {
		...
		update_db(db_client, (op = get_dm_orch_type(radio_cap)), radio_cap.get_radio_cap_info());
...
dm_orch_type_t dm_radio_cap_list_t::get_dm_orch_type(const dm_radio_cap_t& radio_cap)
{
    dm_radio_cap_t *pradio_cap;
...
void dm_radio_cap_list_t::sync_db(db_client_t& db_client, void *ctx)
{
