    ...
    if (pdev != NULL) {
        ...
        if (*pdev == dev) { 
            ...
            if (compare_db(db_client, dev) != true) {
                printf("%s:%d: dev and DB mismatch, needs update\n", __func__, __LINE__);
...
int dm_device_list_t::update_db(db_client_t& db_client, dm_orch_type_t op, void *data)
{
    ...
    switch (op) {
        case dm_orch_type_db_insert:
            printf("    Inserting Device, info->id.net_id: %s\n", mac_str);
            printf("    Interface mac %s\n", dm_easy_mesh_t::macbytes_to_string(info->intf.mac, alid_str));
            printf("    Backhaul alid mac %s\n", dm_easy_mesh_t::macbytes_to_string(info->backhaul_alid.mac, alid_str));
            printf("    Backhaul mac %s\n", dm_easy_mesh_t::macbytes_to_string(info->backhaul_mac.mac, alid_str));
            ...
            ret = insert_row(db_client, key, mac_str,
                    ...
                    mac_str, bsta_str, info->traffic_sep_cap, info->easy_conn_cap,
                    info->test_cap);
            break;

        case dm_orch_type_db_update:
            printf("    Updating Device: %s\n", dm_easy_mesh_t::macbytes_to_string(info->backhaul_mac.mac, bk_mac_str));
                dm_easy_mesh_t::macbytes_to_string(info->backhaul_alid.mac, alid_str);
            printf("    Updating backhual_alid.mac %s\n\n", alid_str);
            printf("    Backhaul sta mac %s\n\n", dm_easy_mesh_t::macbytes_to_string(info->backhaul_sta, bsta_str));
