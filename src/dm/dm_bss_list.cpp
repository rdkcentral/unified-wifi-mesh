...
int dm_bss_list_t::get_config(cJSON *obj_arr, void *parent)
{
	dm_bss_t *pbss;
	cJSON *obj, *akms_arr;
    ...
    while (pbss != NULL) {
		...
		pbss = (dm_bss_t *)hash_map_get_next(m_list, pbss);
    }
...
int dm_bss_list_t::set_config(db_client_t& db_client, const cJSON *obj_arr, void *parent_id)
{
    ...
    for (i = 0; i < size; i++) {
		...
		update_db(db_client, (op = get_dm_orch_type(bss)), bss.get_bss_info());
		update_list(bss, op);
    }
...
dm_orch_type_t dm_bss_list_t::get_dm_orch_type(const dm_bss_t& bss)
{
    dm_bss_t *pbss;
	...
	pbss = (dm_bss_t *)hash_map_get(m_list, key);
...
void dm_bss_list_t::update_list(const dm_bss_t& bss, dm_orch_type_t op)
{
    ...
    switch (op) {
        case dm_orch_type_bss_insert:
			hash_map_put(m_list, strdup(key), new dm_bss_t(bss));	
            break;
        ...
        case dm_orch_type_bss_delete:
			pbss = (dm_bss_t *)hash_map_remove(m_list, key);
            delete(pbss);
            break;
    }
...
void dm_bss_list_t::sync_db(db_client_t& db_client, void *ctx)
{
	em_bss_info_t info;
	mac_addr_str_t	mac;
