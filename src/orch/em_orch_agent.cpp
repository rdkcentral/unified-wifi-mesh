...
bool em_orch_agent_t::is_em_ready_for_orch_exec(em_cmd_t *pcmd, em_t *em)
{
    ...
    } else if (pcmd->m_type == em_cmd_type_get_link_stats_alarm_report) {
        if ((em->get_state() >= em_state_agent_topo_synchronized) ||
            ((em->get_state() == em_state_agent_link_stats_report_pending))) {
            return true;
        }
    }
		...
		dm_easy_mesh_t::macbytes_to_string(pcmd->get_data_model()->get_radio(num)->get_radio_info()->intf.mac, src_mac_str);
            ...
            case em_cmd_type_get_link_stats_alarm_report:
                if ((em->is_al_interface_em() == true)) {
                    queue_push(pcmd->m_em_candidates, em);
                    em_printfout("  em_cmd_type_get_link_stats_alarm_report build candidates pushed\n");
                    count++;
                }
                break;
