...
* | 10 | Free allocated memory for parent_id | free(parent_id) | Should be successful | |
*/
...
* | 03| Verify that the new instance has the same policy as the original modified instance | ASSERT_EQ(policy2.m_policy.num_sta, 5) | The policy of the new instance matches the original | Should Pass |
*/
TEST(dm_policy_tTest, CopyConstructorWithModifiedPolicy) {
    ...
    ASSERT_EQ(policy2.m_policy.num_sta, 5);
...
*/
/*code doesn't handle null
TEST(dm_policy_tTest, CopyConstructorWithNullPolicy) {
...
* | 03 | Verify the internal policy of dm_policy_t matches the provided em_policy_t object | dm_policy.m_policy, valid_policy | ASSERT_EQ should pass | Should Pass |
*/
TEST(dm_policy_tTest, InitializeWithValidEmPolicyTObject) {
    ...
    ASSERT_EQ(dm_policy.m_policy.sta_traffic_stats, valid_policy.sta_traffic_stats);
    ASSERT_EQ(dm_policy.m_policy.sta_link_metric, valid_policy.sta_link_metric);
    ASSERT_EQ(dm_policy.m_policy.sta_status, valid_policy.sta_status);
    ASSERT_EQ(dm_policy.m_policy.independent_scan_report, valid_policy.independent_scan_report);
    ASSERT_EQ(dm_policy.m_policy.profile_1_sta_disallowed, valid_policy.profile_1_sta_disallowed);
    ASSERT_EQ(dm_policy.m_policy.profile_2_sta_disallowed, valid_policy.profile_2_sta_disallowed);
    ASSERT_EQ(dm_policy.m_policy.num_sta, valid_policy.num_sta);
 ...
 * | 03               | Assert equality of dm_policy.m_policy and invalid_mac_policy       | ASSERT_EQ(dm_policy.m_policy, invalid_mac_policy) | Assertion should be true              | Should Pass |
 */
TEST(dm_policy_tTest, InitializeWithEmPolicyTObjectHavingInvalidMacAddressT) {
    ...
    for (int i = 0; i < EM_MAX_STA_PER_STEER_POLICY; ++i) {
        for (int byte = 0; byte < 6; ++byte) {
            ASSERT_EQ(dm_policy.m_policy.sta_mac[i][byte], invalid_mac_policy.sta_mac[i][byte])
...
*/
TEST(dm_policy_tTest, RetrieveValidEmPolicyObject) {
    ...
    ASSERT_EQ(valid_policy.sta_traffic_stats, retrieved_policy->sta_traffic_stats);
    ASSERT_EQ(valid_policy.sta_link_metric, retrieved_policy->sta_link_metric);
    ASSERT_EQ(valid_policy.sta_status, retrieved_policy->sta_status);
    ASSERT_EQ(valid_policy.independent_scan_report, retrieved_policy->independent_scan_report);
    ASSERT_EQ(valid_policy.profile_1_sta_disallowed, retrieved_policy->profile_1_sta_disallowed);
    ASSERT_EQ(valid_policy.profile_2_sta_disallowed, retrieved_policy->profile_2_sta_disallowed);
    ASSERT_EQ(valid_policy.num_sta, retrieved_policy->num_sta);
...
* It ensures that the dm_policy_t object correctly handles the null input and that the get_policy() method 
* returns nullptr as expected.
*
* **Test Group ID:** Basic: 01@n
* **Test Case ID:** 010@n
* **Priority:** High@n
...
* | Variation / Step | Description | Test Data |Expected Result |Notes |
* | :----: | --------- | ---------- |-------------- | ----- |
* | 01| Initialize dm_policy_t with null em_policy_t object | null_policy = nullptr | dm_policy.get_policy() == nullptr | Should Pass |
*/
/*code doesn't handle null - also is it valid to test get method with null handle - doubt
TEST(dm_policy_tTest, InitializeWithNullEmPolicyTObject) {
    ...
    ASSERT_EQ(dm_policy.get_policy(), nullptr);
    std::cout << "Exiting InitializeWithNullEmPolicyTObject" << std::endl;
}
*/

/**
* @brief Test the initialization of dm_policy_t object with an em_policy_t object having invalid MAC addresses.
*
* This test checks the behavior of the dm_policy_t constructor when it is initialized with an em_policy_t object that contains invalid MAC addresses. The objective is to ensure that the dm_policy_t object correctly references the provided em_policy_t object, even if the MAC addresses are invalid.
*
* **Test Group ID:** Basic: 01@n
* **Test Case ID:** 011@n
* **Priority:** High@n
...
* | Variation / Step | Description | Test Data | Expected Result | Notes |
* | :----: | --------- | ---------- |-------------- | ----- |
* | 01 | Initialize em_policy_t with invalid MAC addresses | invalid_mac_policy = {0xDE, 0xAD, 0xBE, 0x00, 0x00, 0x00} | em_policy_t object should be initialized with invalid MAC addresses | Should be successful |
* | 02 | Initialize dm_policy_t with the invalid em_policy_t object | dm_policy(&invalid_mac_policy) | dm_policy_t object should reference the invalid em_policy_t object | Should Pass |
* | 03 | Verify the policy in dm_policy_t | dm_policy.get_policy() | Should return the reference to invalid_mac_policy | Should Pass |
*/
TEST(dm_policy_tTest, InitializeWithEmPolicyTObjectHavingInvalidMacAddress) {
    std::cout << "Entering InitializeWithEmPolicyTObjectHavingInvalidMacAddress" << std::endl;
    em_policy_t invalid_mac_policy;
    memset(&invalid_mac_policy, 0, sizeof(em_policy_t));
    // Set "invalid" MAC addresses — only first 3 bytes populated, rest zero
    for (int i = 0; i < EM_MAX_STA_PER_STEER_POLICY; ++i) {
        invalid_mac_policy.sta_mac[i][0] = 0xDE;
        invalid_mac_policy.sta_mac[i][1] = 0xAD;
        invalid_mac_policy.sta_mac[i][2] = 0xBE;
        invalid_mac_policy.sta_mac[i][3] = 0x00;
        invalid_mac_policy.sta_mac[i][4] = 0x00;
        invalid_mac_policy.sta_mac[i][5] = 0x00;
    }
    // Construct dm_policy with a pointer to the policy
    dm_policy_t dm_policy(&invalid_mac_policy);
    // Validate that the MAC addresses were copied correctly
    em_policy_t *result_policy = dm_policy.get_policy();
    for (int i = 0; i < EM_MAX_STA_PER_STEER_POLICY; ++i) {
        for (int byte = 0; byte < 6; ++byte) {
            ASSERT_EQ(result_policy->sta_mac[i][byte], invalid_mac_policy.sta_mac[i][byte])
                << "MAC mismatch at sta_mac[" << i << "][" << byte << "]";
        }
    }
    std::cout << "Exiting InitializeWithEmPolicyTObjectHavingInvalidMacAddress" << std::endl;
}
...
* | 02| Call the encode function with the valid JSON object and invalid policy ID | policy.encode(&obj, static_cast<em_policy_id_type_t>(-1)) | Should Pass | |
*/
...
TEST(dm_policy_t_Test, RetrievePolicyAfterInitialization) {
    ...
    ASSERT_STREQ(policy->id.net_id, "TestNetID");
    ASSERT_EQ(policy->id.dev_mac[0], 0x01);
    ASSERT_EQ(policy->id.radio_mac[0], 0x02);
    ASSERT_EQ(policy->id.type, em_policy_id_type_steering_local);
    ASSERT_EQ(policy->num_sta, 5);
    ASSERT_EQ(policy->sta_mac[0][0], 0x03);
    ASSERT_EQ(policy->policy, em_steering_policy_type_rcpi_allowed);
    ASSERT_EQ(policy->util_threshold, 50);
    ASSERT_EQ(policy->rcpi_threshold, 60);
    ASSERT_EQ(policy->interval, 10);
    ASSERT_EQ(policy->rcpi_hysteresis, 5);
    ASSERT_TRUE(policy->sta_traffic_stats);
    ASSERT_TRUE(policy->sta_link_metric);
    ASSERT_TRUE(policy->sta_status);
    ASSERT_STREQ(policy->managed_sta_marker, "TestMarker");
    ASSERT_TRUE(policy->independent_scan_report);
    ASSERT_FALSE(policy->profile_1_sta_disallowed);
    ASSERT_FALSE(policy->profile_2_sta_disallowed);
...
TEST(dm_policy_t_Test, InitializePolicyStructureSuccessfully) {
    ...
    ASSERT_EQ(memcmp(&policy.m_policy, &expected_policy, sizeof(em_policy_t)), 0);
...
*/
TEST(dm_policy_tTest, CompareIdenticalObjects) {
...
*/
TEST(dm_policy_tTest, CompareDifferentNetId) {
...
*/
TEST(dm_policy_tTest, CompareDifferentDevMac) {
...
* | 04 | Compare obj1 and obj2 | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentRadioMac) {
...
*/
TEST(dm_policy_tTest, CompareDifferentType) {
...
*/
TEST(dm_policy_tTest, CompareDifferentNumSta) {
...
*/
TEST(dm_policy_tTest, CompareDifferentStaMac) {
...
*/
TEST(dm_policy_tTest, CompareDifferentPolicy) {
...
* | 02 | Compare the two objects using the equality operator | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentUtilThreshold) {
...
*/
TEST(dm_policy_tTest, CompareDifferentRcpiThreshold) {
...
* | 02 | Compare the two objects using the equality operator | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentInterval) {
...
*/
TEST(dm_policy_tTest, CompareDifferentRcpiHysteresis) {
...
* | 02 | Compare the two objects using the equality operator | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentStaTrafficStats) {
...
*/
TEST(dm_policy_tTest, CompareDifferentStaLinkMetric) {
...
*/
TEST(dm_policy_tTest, CompareDifferentStaStatus) {
...
* | 04 | Compare obj1 and obj2 | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentManagedStaMarker) {
...
* | 02 | Compare the two objects using the equality operator | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentIndependentScanReport) {
...
* | 02| Compare the two objects using the equality operator | obj1 == obj2 | EXPECT_FALSE(obj1 == obj2) | Should Pass |
*/
TEST(dm_policy_tTest, CompareDifferentProfile1StaDisallowed) {
...
*/
TEST(dm_policy_tTest, CompareDifferentProfile2StaDisallowed) {
...
* | Variation / Step | Description | Test Data | Expected Result | Notes |
* | :----: | --------- | ---------- |-------------- | ----- |
* | 01 | Call parse_dev_radio_mac_from_key with valid key | key = "device_radio_mac:01:23:45:67:89:AB", id | result = 0, id.radio_mac = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB} | Should Pass |
*/
TEST(dm_policy_test, ValidKeyWithCorrectMACAddressFormat) {
    std::cout << "Entering ValidKeyWithCorrectMACAddressFormat" << std::endl;
    const char *key = "device_radio_mac:01:23:45:67:89:AB";
    em_policy_id_t id;
...
* | Variation / Step | Description | Test Data |Expected Result |Notes |
* | :----: | --------- | ---------- |-------------- | ----- |
* | 01| Call parse_dev_radio_mac_from_key with a key that does not contain a MAC address | key = "device_radio_mac:", id = uninitialized | result = -1, EXPECT_EQ(result, -1) | Should Fail |
*/
TEST(dm_policy_test, KeyWithoutMACAddress) {
    std::cout << "Entering KeyWithoutMACAddress" << std::endl;
    const char *key = "device_radio_mac:";
    em_policy_id_t id;
    int result = dm_policy_t::parse_dev_radio_mac_from_key(key, &id);
    EXPECT_EQ(result, -1);
...
* | Variation / Step | Description | Test Data | Expected Result | Notes |
* | :----: | --------- | ---------- |-------------- | ----- |
* | 01 | Call parse_dev_radio_mac_from_key with invalid MAC address format | key = "device_radio_mac:01:23:45:67:89", id = (uninitialized) | result = -1, EXPECT_EQ(result, -1) | Should Fail |
*/
TEST(dm_policy_test, KeyWithInvalidMACAddressFormat) {
    std::cout << "Entering KeyWithInvalidMACAddressFormat" << std::endl;
    const char *key = "device_radio_mac:01:23:45:67:89";
    em_policy_id_t id;
    int result = dm_policy_t::parse_dev_radio_mac_from_key(key, &id);
    EXPECT_EQ(result, -1);
...
*/
/*code doesn't handle null
TEST(dm_policy_test, NullKeyInput) {
...
* | Variation / Step | Description | Test Data | Expected Result | Notes |
* | :----: | --------- | ---------- |-------------- | ----- |
* | 01 | Call parse_dev_radio_mac_from_key with a null ID | key = "device_radio_mac:01:23:45:67:89:AB", id = nullptr | result = -1, EXPECT_EQ(result, -1) | Should Fail |
*/
/*code doesn't handle null
TEST(dm_policy_test, NullIdInput) {
...
*/
TEST(dm_policy_test, EmptyKeyString) {
