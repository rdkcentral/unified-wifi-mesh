...
TEST(dm_device_t_Test, RetrieveALInterfaceWithEmptyMACAddress) {
    ...
    for (size_t i = 0; i < sizeof(mac_address_t); ++i) {
        ASSERT_EQ(al_interface->mac[i], 0);
    }
    delete device;
...
TEST(dm_device_t_Test, RetrieveALInterfaceWithEmptyInterfaceName) {
    ...
    em_interface_t* al_interface = device->get_al_interface();
    ASSERT_NE(al_interface, nullptr);
    ASSERT_STREQ(al_interface->name, "");
    delete device;
...
TEST(dm_device_t_Test, RetrieveALInterfaceMACAddressWithValidMACAddress) {
    ...
    for (size_t i = 0; i < sizeof(expected_mac); ++i) {
        ASSERT_EQ(mac[i], expected_mac[i]);
    }
    delete device;
...
TEST(dm_device_t_Test, GetDevInterface_InvalidInterface) {
    ...
    // Create and populate device info
    em_device_info_t device_info = {};
    device_info.intf.name[0] = '\0';
    memset(device_info.intf.mac, 0, sizeof(mac_address_t));
    device_info.intf.media = em_media_type_ieee80211b_24;
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    em_interface_t* intf = device->get_dev_interface();
    ASSERT_EQ(intf, nullptr);
    ASSERT_STREQ(intf->name, "");
    for (size_t i = 0; i < sizeof(mac_address_t); ++i) {
        ASSERT_EQ(intf->mac[i], 0) << "MAC byte at index " << i << " is not zero";
    }
    ASSERT_EQ(intf->media, device_info.intf.media);
    delete device;
    std::cout << "Exiting GetDevInterface_InvalidInterface test\n";
}
...
TEST(dm_device_t_Test, RetrieveMACAddressWithValidMACAddressSet) {
    ...
    ASSERT_NE(mac, nullptr);
    ASSERT_EQ(memcmp(mac, valid_mac, sizeof(valid_mac)), 0);
    delete device;
...
TEST(dm_device_t_Test, RetrieveMACAddressWithMaximumLengthMACAddress) {
    ...
    unsigned char* mac = device->get_dev_interface_mac();
    ASSERT_NE(mac, nullptr);
    ASSERT_EQ(memcmp(mac, max_length_mac, 6), 0);
    delete device;
...
TEST(dm_device_t_Test, RetrieveMACAddressWithMinimumLengthMACAddress) {
    ...
    unsigned char* mac = device->get_dev_interface_mac();
    ASSERT_NE(mac, nullptr);
    ASSERT_EQ(memcmp(mac, min_length_mac, 6), 0);
    delete device;
...
TEST(dm_device_t_Test, RetrieveMACAddressWithInvalidCharactersInMACAddress) {
    ...
    unsigned char* mac = device->get_dev_interface_mac();
    ASSERT_NE(mac, nullptr);
    ASSERT_EQ(memcmp(mac, invalid_mac, 6), 0);
    delete device;
 ...
 * | 02 | Initialize the device | None | None | Should be successful |
 * | 03 | Set the device interface name to "eth0" | device->set_dev_interface_name("eth0") | None | Should be successful |
 * | 04 | Retrieve the device interface name | char* result = device->get_dev_interface_name() | result = "eth0" | Should Pass |
 * | 05 | Verify the retrieved interface name is "eth0" | ASSERT_STREQ(result, "eth0") | None | Should Pass |
 ...
 * | 03 | Verify the retrieved name matches the set name | None | ASSERT_STREQ(result, "!@#$%^&*()") | Should Pass |
 */
 ...
 */
 TEST(dm_device_t_Test, RetrieveDeviceInfoAfterSettingValidFields) {
    ...
    ASSERT_EQ(memcmp(returned_info->intf.mac, mac, sizeof(mac)), 0);
    ASSERT_STREQ(returned_info->intf.name, interface_name);
    ASSERT_STREQ(returned_info->manufacturer, manufacturer);
    ASSERT_STREQ(returned_info->software_ver, software_version);
    ASSERT_STREQ(returned_info->serial_number, serial_number);
    delete local_device;
 ...
 * | 04 | Verify MAC address is initialized to 0 | ASSERT_EQ(device_info->intf.mac[0], 0) | device_info->intf.mac[0] = 0 | Should Pass |
 * | 05 | Verify interface name is initialized to empty string | ASSERT_EQ(device_info->intf.name[0], '\0') | device_info->intf.name[0] = '\0' | Should Pass |
 * | 06 | Verify manufacturer is initialized to empty string | ASSERT_EQ(device_info->manufacturer[0], '\0') | device_info->manufacturer[0] = '\0' | Should Pass |
 * | 07 | Verify software version is initialized to empty string | ASSERT_EQ(device_info->software_ver[0], '\0') | device_info->software_ver[0] = '\0' | Should Pass |
 * | 08 | Verify serial number is initialized to empty string | ASSERT_EQ(device_info->serial_number[0], '\0') | device_info->serial_number[0] = '\0' | Should Pass |
 */
TEST(dm_device_t_Test, RetrieveDeviceInfoAfterSettingNullStructure) {
    ...
    ASSERT_EQ(device_info->intf.mac[0], 0);
    ASSERT_EQ(device_info->intf.name[0], '\0');
    ASSERT_EQ(device_info->manufacturer[0], '\0');
    ASSERT_EQ(device_info->software_ver[0], '\0');
    ASSERT_EQ(device_info->serial_number[0], '\0');
 ...
 * | 03 | Assert that the get type is within expected enum values | ASSERT_GE(type, dm_orch_type_none), ASSERT_LE(type, dm_orch_type_beacon_report) | None | Should Pass |
 */
TEST(dm_device_t_Test, RetrieveOrchType) {
    ...
    ASSERT_GE(type, dm_orch_type_none);
    ASSERT_LE(type, dm_orch_type_beacon_report);
...
TEST(dm_device_t_Test, RetrieveManufacturerWithValidManufacturerName) {
    ...
    ASSERT_STREQ(manufacturer, "ValidManufacturer");
    delete device;
 ...
 * | 03 | Verify the retrieved manufacturer name is an empty string | ASSERT_STREQ(manufacturer, ""); | None | Should Pass |
 */
 ...
 * | 03 | Verify the retrieved manufacturer name matches the expected value | ASSERT_STREQ(manufacturer, "!@#$%^&*()_+") | None | Should Pass |
 */
...
TEST(dm_device_t_Test, RetrievePrimaryDeviceType_Switch) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_primary_device_type(), "Switch");
    delete device;
...
TEST(dm_device_t_Test, RetrievePrimaryDeviceType_EmptyString) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_primary_device_type(), "");
    delete device;
...
TEST(dm_device_t_Test, RetrievePrimaryDeviceType_SpecialCharacters) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_primary_device_type(), "!@#$%^&*()_+");
    delete device;
...
TEST(dm_device_t_Test, RetrievePrimaryDeviceType_SpacesAndNumbers) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_primary_device_type(), "Primary Device Type123");
    delete device;
...
TEST(dm_device_t_Test, RetrieveSerialNumberWhenSet) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_serial_number(), "1234567890");
    delete device;
...
TEST(dm_device_t_Test, RetrieveSerialNumberWhenSetToEmptyString) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_serial_number(), "");
    delete device;
...
TEST(dm_device_t_Test, RetrieveSerialNumberWhenSetToSpecialCharacters) {
    ...
    // Instantiate device with populated info
    dm_device_t* device = new dm_device_t(&device_info);
    ASSERT_STREQ(device->get_serial_number(), u8"SN@#123!$%\n\t\u00A9");
    delete device;
...
TEST(dm_device_t_Test, InitializeDeviceInformationStructureAfterModifyingDeviceInfo) {
    ...
    int result = device.init();
    ASSERT_EQ(result, 0);
    ASSERT_EQ(device.m_device_info.manufacturer[0], '\0');
 ...
 * | 03 | Verify that the function returns -1, indicating an error. | result = -1 | ASSERT_EQ(result, -1) | Should Pass |
 */
 ...
 * | 03 | Verify the result is -1 | result = -1 | ASSERT_EQ(result, -1) | Should Pass |
 */
 ...
 * | 02 | Verify the result is as expected | result = 0 | ASSERT_EQ(result, 0) | Should be successful |
 */
 ...
 * | 02 | Verify that the result is 0 using ASSERT_EQ. | result = 0 | Assertion should pass | Should be successful |
 */
...
TEST(dm_device_t_Test, IdenticalDeviceInformation) {
    ...
    device1.m_device_info.id.media = em_media_type_ieee8023ab;
    device2.m_device_info.id.media = em_media_type_ieee8023ab;
    ...
    device2.m_device_info.intf.media = em_media_type_ieee8023ab;
    device1.m_device_info.profile = em_profile_type_1;
    device2.m_device_info.profile = em_profile_type_1;
...
TEST(dm_device_t_Test, DifferentNetId) {
    ...
    device2.m_device_info = device1.m_device_info;
...
TEST(dm_device_t_Test, AssigningValidDeviceInformation) {
    ...
    obj1 = obj2;
    ASSERT_EQ(obj1.m_device_info.id.media, obj2.m_device_info.id.media);
    ASSERT_EQ(obj1.m_device_info.intf.media, obj2.m_device_info.intf.media);
    ASSERT_EQ(obj1.m_device_info.profile, obj2.m_device_info.profile);
...
TEST(dm_device_t_Test, AssigningNullDeviceInformation) {
    ...
    dm_device_t obj2;
    obj1 = obj2;
    ASSERT_EQ(0, memcmp(&obj1.m_device_info, &obj2.m_device_info, sizeof(em_device_info_t)));
...
TEST(dm_device_t_Test, InitializeWithValidDeviceInfo) {
    ...
    ASSERT_EQ(device.m_device_info.id.media, em_media_type_ieee8023ab);
    ASSERT_EQ(device.m_device_info.intf.media, em_media_type_ieee8023ab);
    ASSERT_EQ(device.m_device_info.profile, em_profile_type_1);
...
TEST(dm_device_t_Test, CopyConstructorWithValidDeviceInformation) {
    ...
    ASSERT_EQ(copiedDevice.m_device_info.report_unsuccess_assocs, true);
    ASSERT_EQ(copiedDevice.m_device_info.prioritization_sup, false);
