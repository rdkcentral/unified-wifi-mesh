 ...
 *It ensures that the method can handle empty input.
 *
 ***Test Group ID:* *Basic: 01
 ***Test Case ID:* *009
 ***Priority:* *High
 ...
 *| Variation / Step | Description | Test Data | Expected Result | Notes |
 *| :----: | --------- | ---------- |-------------- | ----- |
 *| 01 | Call the `deserialize` method with an empty data vector | data = {} | Method should handle empty input | Should Pass |
 */
TEST(AlServiceDataUnitTest, DeserializeWithEmptyDataVector) {
    ...
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_ANY_THROW(alServiceDataUnit.deserialize(data));
...
TEST(AlServiceDataUnitTest, DeserializeWithValidDataVector) {
    ...
    std::vector<unsigned char> data(25, 0x01);
    AlServiceDataUnit alServiceDataUnit;
...
TEST(AlServiceDataUnitTest, DeserializeWithMaximumSizeDataVector) {
    ...
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_NO_THROW(alServiceDataUnit.deserialize(data));
...
TEST(AlServiceDataUnitTest, DeserializeWithDataVectorContainingOnlyZeros) {
    ...
    std::vector<unsigned char> data = {10, 0x00};
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_ANY_THROW(alServiceDataUnit.deserialize(data));
...
TEST(AlServiceDataUnitTest, DeserializeWithDataVectorContainingOnlyOnes) {
    ...
    std::vector<unsigned char> data = {20, 0x01};
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_ANY_THROW(alServiceDataUnit.deserialize(data));
...
TEST(AlServiceDataUnitTest, DeserializeWithDataVectorContainingValidMacAddressesAndPayload) {
    ...
    while (data.size() < PACKET_HEADER_SIZE) data.push_back(0x50);
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_NO_THROW(alServiceDataUnit.deserialize(data));
 ...
 *| Variation / Step | Description | Test Data | Expected Result | Notes |
 *| :----: | --------- | ---------- |-------------- | ----- |
 *| 01 | Create a data vector containing invalid MAC addresses | data = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x01, 0x02, 0x10, 0x11, 0x12, 0x13} | Data vector should be created successfully | Should be successful |
 *| 02 | Call the `deserialize` method with the invalid data vector | data = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x01, 0x02, 0x10, 0x11, 0x12, 0x13} | Method should handle the input without crashing | Should Pass |
 */
TEST(AlServiceDataUnitTest, DeserializeWithDataVectorContainingInvalidMacAddresses) {
    ...
    while (data.size() < PACKET_HEADER_SIZE) data.push_back(0x00);
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_NO_THROW(alServiceDataUnit.deserialize(data));
 ...
 *| Variation / Step | Description | Test Data |Expected Result |Notes |
 *| :----: | --------- | ---------- |-------------- | ----- |
 *| 01| Retrieve the source AL MAC address using getSourceAlMacAddress method and print it| None | MAC address should be valid | Should Pass |
 */
 ...
 */
 TEST(AlServiceDataUnitTest, SetDestinationMacAddress_InvalidFormat1) {
    ...
    AlServiceDataUnit alServiceDataUnit;
    EXPECT_ANY_THROW(alServiceDataUnit.setDestinationAlMacAddress(parseMacAddress(macAddress)));
}
...
TEST(AlServiceDataUnitTest, SetInvalidMacAddressShortLength) {
    ...
    EXPECT_ANY_THROW({
        alServiceDataUnit.setSourceAlMacAddress(parseMacAddress(macAddress));
        std::cout << "Expected exception for invalid MAC format (too short)" << std::endl;
    });
...
/**
 * @brief Verify that the default constructed instance of AlServiceDataUnit initializes its source MAC address to zero.
 *
 * This test validates that a default constructed AlServiceDataUnit object correctly initializes its source MAC address by ensuring that each element is zero. The test creates an object, retrieves the MAC address using the getSourceAlMacAddress method, and iterates through the address array to validate each element.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 049@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Construct an instance of AlServiceDataUnit using the default constructor | input: none, output: instance created without exceptions | Object is constructed without exceptions | Should be successful |
 * | 02 | Invoke getSourceAlMacAddress() on the constructed object to retrieve the MAC address | input: none, output: MAC address (an array of bytes) | MAC address array is returned with valid size | Should Pass |
 */
TEST(AlServiceDataUnit, RetrieveDefaultSourceAlMacAddress) {
    std::cout << "Entering RetrieveDefaultSourceAlMacAddress test" << std::endl;
    EXPECT_NO_THROW(AlServiceDataUnit unit);
    AlServiceDataUnit unit;
    std::cout << "Default constructed AlServiceDataUnit object created." << std::endl;
    std::cout << "Invoking getSourceAlMacAddress() method." << std::endl;
    const MacAddress& macAddress = unit.getSourceAlMacAddress();
    std::cout << "Retrieved MAC address values: ";
    for (size_t i = 0; i < macAddress.size(); i++) {
        std::cout << static_cast<int>(macAddress[i]) << " ";
    }
    std::cout << std::endl;
    std::cout << "Exiting RetrieveDefaultSourceAlMacAddress test" << std::endl;
}
