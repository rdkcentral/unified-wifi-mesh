...
sudo ip link del $VETH_END 2>/dev/null
...
sudo ip link add name $VETH_END type veth peer name $VETH_PEER
sudo ip link set $VETH_END master $BRIDGE_NAME
sudo ip link set dev $VETH_END up
...
if [ "$DO_OBTAIN_MAC" = true ]; then
    ...
    sudo ip link set dev $VETH_PEER address $veth0_mac
fi
...
sudo timeout 3 socat -u UDP4-RECVFROM:$TEST_PORT,broadcast - > $VETH_OUTPUT &
LISTENER_PID=$!
sleep 1

# Send broadcast from veth peer
echo "$TEST_MSG" | sudo socat - UDP4-DATAGRAM:255.255.255.255:$TEST_PORT,broadcast,bindtodevice="$VETH_PEER"
